...
local function createQuickSettingsPanel(touch_menu)
    ...
    local small_btn_width = Screen:scaleBySize(45) -- wider buttons
    ...
    if config.show_frontlight then
        ...
        local fl_progress -- defined below
        ...
        fl_progress = ProgressWidget:new{
            width = slider_width,
            height = btn_height,
            percentage = fl.cur / math.max(1, fl.max),
            show_perc = false,
            padding = 0,
            margin_h = 0,
            margin_v = 0,
    ...
    if config.show_warmth and Device:hasNaturalLight() then
        ...
        local nl_progress = ProgressWidget:new{
            width = warmth_slider_width,
            height = btn_height_warmth,
            percentage = (nl.cur - nl.min) / math.max(1, nl.max - nl.min),
            show_perc = false,
            padding = 0,
            margin_h = 0,
            margin_v = 0,
        ...
        local function setWarmth(warmth)
            ...
            if nl_progress and nl_progress.setPercentage then
                nl_progress:setPercentage((nl.cur - nl.min) / math.max(1, nl.max - nl.min))
            end
...
local function handlePanelGesture(touch_menu, ges)
    ...
    -- Check warmth progress bar
    if refs.nl_progress and refs.nl_progress.dimen
       and ges.pos:intersectWith(refs.nl_progress.dimen) then
        local perc = refs.nl_progress:getPercentageFromPosition(ges.pos)
        if perc and refs.setWarmth then
